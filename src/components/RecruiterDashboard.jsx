...
export default function RecruiterDashboard() {
  ...
  React.useEffect(() => {
    init();
    const ch = supabase.channel("recruiter-live")
  ...
  const init = async () => {
    ...
    await Promise.all([fetchSubmissions(ids),fetchAppCounts(ids),fetchPipeline(ids)]);
  };
  ...
  const fetchApplications = async (jobId) => {
    ...
    const { data:profs } = await supabase.from("profiles").select("id,full_name,email,skills,bio").in("id",apps.map(a=>a.applicant_id));
  ...
  const fetchPipeline = async (ids) => {
    ...
    const { data:profs } = await supabase.from("profiles").select("id,full_name,email").in("id",apps.map(a=>a.applicant_id));
    const pm = Object.fromEntries((profs||[]).map(p=>[p.id,p]));
  ...
  const updatePipelineStatus = async (appId,newStatus) => {
  ...
  const deleteJob = async (jobId) => {
    ...
    jobIdsRef.current = ids;
    await Promise.all([fetchAppCounts(ids),fetchPipeline(ids),fetchSubmissions(ids)]);
  ...
  const postJob = async (e) => {
    ...
    const { data, error } = await supabase.from("jobs").insert({...jobForm,posted_by:user.id,status:"active"}).select().single();
  ...
  return (
    <>
          ...
          <div className="rdModal" onClick={e=>e.stopPropagation()}>
            ...
            <form onSubmit={saveEditJob} style={{display:"flex",flexDirection:"column",gap:14}}>
              <div className="rdFormRow">
                <div className="rdField"><label>Job Title *</label><input required value={editForm.title||""} onChange={e=>setEditForm(f=>({...f,title:e.target.value}))} /></div>
        ...
        <div className="rdContent">
            ...
            <div className="rdPanel">
              ...
              <div className="rdSection">
                <div className="rdSectionHead">
                  ...
                  <button className="rdLinkBtn" onClick={()=>setTab("pipeline")}>Full pipeline →</button>
                </div>
                <div style={{display:"flex",gap:0,overflow:"hidden"}}>
                  {STAGES.filter(s=>s!=="rejected").map(s=>(
                    <div key={s} style={{flex:1,padding:"14px 12px",textAlign:"center",borderRight:"1px solid rgba(255,255,255,0.05)"}}>
                      <div style={{fontSize:18,fontWeight:900,color:SC[s]}}>{(pipeline[s]||[]).length}</div>
                      <div style={{fontSize:11,color:"rgba(245,245,245,0.4)",fontWeight:600,textTransform:"capitalize",marginTop:3}}>{s}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="rdPanel">
              {tab==="applicants"&&selectedJob ? (
                <>
                  ...
                  {applications.length===0 ? <div className="rdEmpty">No applications yet.</div> : (
                        ...
                        <div key={app.id}>
                          <div className="rdAppRow" onClick={()=>setExpandedApp(expandedApp===app.id?null:app.id)}>
                            ...
                            <div className="rdAppActions">
                              <select className="rdStatusSelect" value={app.status} style={{"--sc":SC[app.status]||"#13b8c8"}}
                                onClick={e=>e.stopPropagation()} onChange={e=>updateStatus(app.id,e.target.value)}>
                          ...
                          {expandedApp===app.id&&(
                            <div className="rdAppExpanded">
                              {app.cover_letter ? <p className="rdCoverText">"{app.cover_letter}"</p> : <p style={{color:"rgba(245,245,245,0.3)",fontSize:13}}>No cover letter provided.</p>}
                              {app.profiles?.bio&&<p style={{margin:"8px 0 0",fontSize:13,color:"rgba(245,245,245,0.5)"}}>{app.profiles.bio}</p>}
                            </div>
                          )}
                        </div>
                      ))}
          ...
          {tab==="pipeline" && (
            <div style={{display:"flex",flexDirection:"column",gap:20}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                ...
                <span style={{fontSize:13,color:"rgba(245,245,245,0.4)"}}>All candidates across all jobs</span>
              </div>
              {totalApps===0 ? <div className="rdEmpty">No applications yet. Share your job links to get started.</div> : (
                    ...
                    <div className="rdKanbanCol" key={stage}>
                      ...
                      <div className="rdKanbanCards">
                        {(pipeline[stage]||[]).length===0 ? (
                          <div style={{padding:"16px 12px",textAlign:"center",fontSize:12,color:"rgba(245,245,245,0.2)"}}>Empty</div>
                        ) : (pipeline[stage]||[]).map(app=>(
                          <div className="rdKanbanCard" key={app.id}>
                            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                              <div className="rdKanbanAvatar">{(app.profiles?.full_name||app.profiles?.email||"?").charAt(0).toUpperCase()}</div>
                              <div style={{flex:1,minWidth:0}}>
                                <p style={{margin:0,fontSize:12,fontWeight:800,color:"#f5f5f5",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{app.profiles?.full_name||"Unknown"}</p>
                                <p style={{margin:0,fontSize:11,color:"rgba(245,245,245,0.4)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{app.profiles?.email||"—"}</p>
                              </div>
                            </div>
                            <p style={{margin:"0 0 8px",fontSize:11,color:"rgba(245,245,245,0.5)",lineHeight:1.4}}>📋 {app.jobs?.title} · {app.jobs?.company}</p>
                            <p style={{margin:"0 0 8px",fontSize:10,color:"rgba(245,245,245,0.3)"}}>{app.created_at?.slice(0,10)}</p>
                            <select className="rdMiniSelect" value={app.status} style={{"--sc":SC[app.status]}}
                              onChange={e=>updatePipelineStatus(app.id,e.target.value)}>
                              {STAGES.map(s=><option key={s} value={s}>{s}</option>)}
                            </select>
                          </div>
                        ))}
          ...
          {tab==="submissions" && (
            <div className="rdPanel">
              ...
              <p style={{margin:"-12px 0 0",fontSize:13,color:"rgba(245,245,245,0.4)"}}>Candidates submitted by vendor partners</p>
      ...
      <style>{`
        .rdToast{position:fixed;bottom:24px;right:24px;background:#1a2035;border:1px solid rgba(255,255,255,0.12);color:#f5f5f5;padding:12px 20px;border-radius:12px;z-index:9999;font-size:14px;font-weight:600;box-shadow:0 8px 32px rgba(0,0,0,0.4);animation:slideUp 0.3s ease;}
        ...
        .rdEditBtn{height:34px;padding:0 14px;border-radius:999px;border:1px solid rgba(247,183,51,0.3);background:rgba(247,183,51,0.08);color:#f7b733;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;transition:all 0.15s;}
        .rdEditBtn:hover{background:rgba(247,183,51,0.14);}
        .rdToggleBtn{height:34px;padding:0 14px;border-radius:999px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:rgba(245,245,245,0.6);font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;transition:all 0.15s;}
        .rdToggleBtn:hover{background:rgba(255,255,255,0.08);color:#f5f5f5;}
        .rdDeleteBtn{height:34px;padding:0 14px;border-radius:999px;border:1px solid rgba(239,68,68,0.25);background:rgba(239,68,68,0.06);color:#f87171;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;transition:all 0.15s;}
        .rdDeleteBtn:hover{background:rgba(239,68,68,0.14);}
        ...
        .rdAppExpanded{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-top:none;border-radius:0 0 12px 12px;padding:14px 18px;}
        ...
        .rdAppInfo{min-width:140px;flex:1;}.rdAppName{margin:0 0 2px;font-size:13.5px;font-weight:800;color:#f5f5f5;}.rdAppEmail{margin:0;font-size:12px;color:rgba(245,245,245,0.4);}
        ...
        /* Kanban */
        .rdKanban{display:flex;gap:14px;overflow-x:auto;padding-bottom:16px;min-height:400px;}
        .rdKanbanCol{flex:0 0 200px;display:flex;flex-direction:column;gap:0;}
        .rdKanbanHead{padding:10px 14px;border-radius:10px 10px 0 0;background:color-mix(in srgb,var(--sc) 14%,rgba(15,19,32,1));border:1px solid color-mix(in srgb,var(--sc) 25%,transparent);border-bottom:none;display:flex;align-items:center;justify-content:space-between;}
        .rdKanbanLabel{font-size:12px;font-weight:800;color:var(--sc);text-transform:capitalize;}
        .rdKanbanCount{font-size:11px;font-weight:900;background:color-mix(in srgb,var(--sc) 20%,transparent);color:var(--sc);padding:2px 8px;border-radius:999px;}
        .rdKanbanCards{flex:1;background:rgba(15,19,32,0.6);border:1px solid rgba(255,255,255,0.07);border-radius:0 0 10px 10px;padding:10px;display:flex;flex-direction:column;gap:8px;overflow-y:auto;max-height:500px;}
        .rdKanbanCard{background:rgba(10,14,26,0.9);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:12px;cursor:default;transition:border-color 0.15s;}
        .rdKanbanCard:hover{border-color:rgba(255,255,255,0.18);}
        .rdKanbanAvatar{width:28px;height:28px;border-radius:50%;background:rgba(19,184,200,0.15);color:#13b8c8;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:900;flex-shrink:0;}
